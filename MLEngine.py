...
class MLEngine:
    def __init__(self,data_path='',file_to_load='',subject_id='',sessions=[1, 2],ntimes=1,kfold=2,m_filters=2,window_details={},mc_methods='OVR'):
    ...
    def evaluation(self):
        ...
        '''for BCIC Dataset'''
        evalData = self.file_to_load.replace("T","E")
        bcic_data = LoadData.LoadBCICE(evalData, self.data_path)
        eeg_data = bcic_data.get_epochs()
        print(eeg_data)
        exit()
        '''for KU dataset'''
        ...
        for k in range(self.ntimes):
            ...
            for i in range(self.kfold):
                ...
                print(f'Training Kappa Score = {str(kappa_train)}')
                print(f'Training F1 Score = {str(f1_train)}')
                print(f'Testing Accuracy = {str(te_acc)}')
                print(f'Testing Kappa Score = {str(kappa_test)}')
                print(f'Testing F1 Score = {str(f1_test)}')
